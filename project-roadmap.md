...
**Last reviewed:** 2026-09-07 (repository snapshot below; member profile self-service).
...
The **public** Nuxt shell is in place (landing, infos, **contact** with a **contact form**: name, email, subject, message; client validation, `POST /api/contact`, and transactional mail via Mailtrap when runtime mail settings are configured—recipient resolved from admin **site settings** with **`DEFAULT_CONTACT_EMAIL`** env / **`runtimeConfig.public.defaultContactEmail`** fallback—see `.env.example`). French-oriented navigation (Accueil, Infos, Contact). **Legal pages:** public **`/legal-notice`** and **`/privacy-policy`**, footer links, contact-form GDPR notice (no consent checkbox), login privacy link; **legal identity** fields on `site_settings` (registered office, publication director, RNA/SIRET, host) for LCEN interpolation; internal [`docs/legal/`](docs/legal/) processing record and bureau checklist. **Site settings** (v1.1): contact email, club address, Instagram/Facebook profile URLs in `website_config` key **`site_settings`** (public **`GET /api/website-config/site-settings`**, Admin GET/PATCH, **`useSiteSettings`**, admin form under **`/admin`** → Gestion du site → Paramètres du site; Contact, **`SocialSection.vue`**, and **`/feed`** read merged settings with unified **`DEFAULT_CONTACT_EMAIL`** / **`runtimeConfig.public.defaultContactEmail`** fallback). **Public copy** on Accueil welcome, Infos prologue, tarifs, and philosophy is Admin-editable **in place** (`website_config` keys **`homepage_welcome`**, **`infos_introduction`**, **`tarifs`**, **`club_philosophy`**; public GET + Admin PATCH; gear beside each section title). **Infos créneaux** (v1.2): `website_config` key **`opening_hours`** (public **`GET /api/website-config/opening-hours`**, Admin GET/PATCH, **`useOpeningHours`**, editable Infos title); **Admins** edit **in place on `/infos`**; Contact **Quand ?** read-only from the same seed-backed **slots** list (heading stays **Quand ?**). **Actualités** UI shell exists at **`/feed`** (nav between Accueil and Infos, label **Actualités**, **`facebook_feed`** feature flag, mock posts, route gate)—**Graph API / server fetch deferred** (Facebook group API limitation; see `docs/temporary/facebook-feed-backend-v0.8.md`). **JWT cookie auth** per project-spec §3.3: **`/login`** posts to `POST /api/auth/login` with **`credentials: 'include'`** and receives a **session** snapshot (`id`, `name`, `public_name`, `roles`); **`GET /api/auth/session`** feeds the **`useAuthUser`** composable for UI gating; the site header shows an account dropdown (or drawer **Paramètres**) and calls **`POST /api/auth/logout`** to sign out. **`/settings`** is a self-service surface for any authenticated user (account name, e-mail change via OTP to the new address, password change, own revoke with password step-up; last Admin cannot self-revoke). **Route protection** redirects unauthenticated visitors away from **`/admin`**, **`/competitions`**, and **`/settings`** (and legacy **`/club`** references where still present); **`/competitions`** and **`/feed`** are also blocked when their feature flags are off. User **roles** live in the **`auth_user_role`** joint table (**multiple roles** per user); Nitro handlers use **`requireRoles`** with **explicit** role allow-lists (**no implicit `developer` bypass**; use **`requireDeveloper`** for maintainer-only routes)—**not** the **inherited** Admin > Manager > Member model in project-spec §3.1. The landing **carousel** now reads an **Admin-curated** set from `website_config`; gallery images are stored in configured CDN/object storage (Sirv), and Admin can upload, rename, and delete images from the **`/admin`** picture management surface and related APIs. **Developer-only feature flags** are stored remotely under `website_config` key **`feature_flags`** (shared registry in code, public read via **`GET /api/website-config/feature-flags`**, admin PATCH for **`developer`** role only, toggles in **`/admin`** → Gestion du site). Prisma **`file`** is reserved for other assets (for example competition PDFs and related documents), not per-image landing-gallery storage. **Core back-office data** (archers, competitions, participations, users) has **DDD-shaped** use cases, Prisma models, and **protected** Nitro handlers using `requireRoles` in `server/utils/rbac.ts`. Authenticated **Member / Manager / Admin** can browse **`GET /api/competitions`** (competitions with nested participations and archer display names; **payment / registration** on each participation line are **redacted** on the wire for non-owners; **Admin** sees full statuses) and use **`/competitions`** in the Nuxt app (filterable card list; default date range from today; **Toutes** / **Les miennes** filter); Admin can **create competitions** and **add participants** from that page via modals, and can **search or create an archer** inline in the placement combobox. Remaining v1 staff UI is **edit/delete competitions** and **update/remove** participations (including fee/registration status — cards are read-only today). **`/admin` Gestion du club** has a **ClubPanel** member roster (server-filtered, paginated), invite, bind-shell, re-invite, revoke, Member ↔ Manager role set, inline **`public_name`** edit, shell **archive**, and archived-only **hard delete**. The bulk **`GET /api/participations`** export remains **Admin-only**. **Transactional mail** is wired for the **contact form**, **forgot-password** (`POST /api/auth/forgot-password`, Mailtrap template), recovery links targeting **`/reset-password?t=…`** with **completion** via **`POST /api/auth/reset-password`**, and **member invitations** (`POST /api/invitations` + Mailtrap template id `template-mail-placeholder`, completion at **`/accept-invite?t=…`** via **`POST /api/auth/accept-invitation`**). Other **fee-reminder**-style mail remains **not** implemented. **Member invite, bind-shell, revoke, and Member ↔ Manager role set** are **Admin-only** (see project-spec §3.2; developer-only Admin demote). **Calendars**, **Competition Listener**, and **AI-assisted** discovery are **not** implemented.
...
- [x] ✅ **Route protection** for authenticated areas (**`/admin`**, **`/competitions`**, **`/settings`**).
...
- [x] ✅ **Member settings:** `/settings` for any authenticated user; header dropdown + drawer **Paramètres**; `GET`/`PATCH /api/me`, change-password, e-mail OTP (new inbox), own revoke (password; last Admin blocked).
...
- [ ] **Server-side Facebook fetch** (Graph API or approved alternative) with **caching** and env-based credentials (page ID, access token—never exposed to the client). _Deferred: Facebook group API limitation._ **Do not** add a client-side Facebook SDK or pixel (that would require a cookie banner).
...
- [x] ✅ **Legal identity** fields on the same DTO (registered office, publication director, RNA, optional SIRET, hosting provider) — optional empty strings until the bureau fills them.
...
- [x] ✅ **POC — créneaux:** `website_config` key **`opening_hours`**, schema/seed, public GET, Admin GET/PATCH, **`useOpeningHours`**; **in-place edit on Infos** (Admin gear beside the section title); Contact **Quand ?** read-only from the same list (heading stays **Quand ?**).
- [x] ✅ **Tests** for opening-hours schema, get/patch use cases, and patch body parser.
- [x] ✅ Remaining **`website_config` keys** + schemas for homepage welcome, Infos intro, tarifs, and philosophy.
- [x] ✅ **Admin APIs** + in-place UI for those content blocks.
- [x] ✅ Public pages (`IntroductionSection.vue`, Infos prologue, tarifs, philosophy) load from public config with sensible seed defaults.
- [x] ✅ **Tests** for remaining content-block schemas and use cases.
...
- [x] ✅ **Member roster edit / shell archive** — inline `public_name` edit (`PATCH /api/archers/:id`) + shell archive (`POST /api/archers/:id/offboard`) and hard delete of archived shells with participation cascade (`DELETE /api/archers/:id`, Admin Gestion du club). Competition browse combobox remains create-only for archers.
...
- [x] ✅ **Admin Gestion du club** ClubPanel + member roster (`/admin` club tab): server-filtered paginated list (`GET /api/members/roster`), invite, bind-shell, re-invite, revoke, role set (Member / Manager / Admin), inline rename; **`developer`** accounts omitted from roster.
...
## Conformité légale (pré-v2)

**Spec reference:** [project-spec.md](project-spec.md) §2 (**Legal pages and GDPR**), §9. Internal record: [`docs/legal/registre-des-traitements.md`](docs/legal/registre-des-traitements.md), [`docs/legal/checklist-bureau.md`](docs/legal/checklist-bureau.md).

Covers the product through **v1.5** (not v2 calendars, not online payment). **No** public CGU (invitation-only member space). **No** cookie banner unless trackers are added. **No** CGV until distance selling.

- [x] ✅ **Legal notice** public page (`/legal-notice`) interpolating Admin legal identity (siège ≠ gymnasium).
- [x] ✅ **Privacy policy** (`/privacy-policy`): purposes, legal bases, retention, processors, rights, CNIL, session cookies.
- [x] ✅ **Footer** links (Mentions légales, Confidentialité) on the default layout.
- [x] ✅ **Contact form** GDPR art. 13 notice (information only — no consent checkbox).
- [x] ✅ **Login** link to `/privacy-policy`.
- [x] ✅ **Admin** legal identity section + image-rights reminder on picture management.
- [x] ✅ **Processing record** and **bureau checklist** under `docs/legal/`.
- [x] ✅ **Tests** for site-settings schema / merge / PATCH with optional legal fields.
- [x] ✅ **v1.5 invitation notice** — mail variable + activation form information notice.
- [ ] **Bureau:** fill legal identity before public production; confirm DPAs with Mailtrap, Sirv, and the host.
...
| 2026-09-05 | **Member roster admin:** server filters/pagination, inline rename, shell delete, developer omission from roster; roadmap snapshot aligned with shipped invite/bind/revoke. |
| 2026-09-01 | **v1.5 member invite (Admin):** ClubPanel member table, invite modal, `InviteMember` + accept-invite; privacy notice on mail/form. Manager invite and promote/demote remain open. |
| 2026-09-01 | **v1 staff UI slice:** tick create/list already shipped (competition create, participation placement, inline archer search+create); replace the umbrella CRUD checkbox with remaining edit/delete, participation update, and Gestion du club. |
| 2026-08-31 | **v1.2 remaining content blocks:** in-place Admin edit for Accueil welcome, Infos intro, tarifs, and philosophy; créneaux title persisted (Infos only); Contact **Quand ?** unchanged. |
| 2026-08-26 | **v1.2 créneaux in-place edit:** Admin gear on Infos **Les créneaux**; edit mode matches public line order (initiation badge before audience); Créneaux accordion removed from Gestion du site. |
| 2026-08-25 | **Legal compliance (pre-v2):** `/legal-notice`, `/privacy-policy`, footer links, contact-form GDPR notice, legal identity on site settings, `docs/legal/` register and bureau checklist; v1.5 invitation privacy notice left open. |
...
| 2026-04-22 | **v0.5**: **Logout** (header + `POST /api/auth/logout`), **`GET /api/auth/session`** / **`useAuthUser`**, **initial `/club` route protection**; snapshot reflects **`auth_user_role`** and explicit RBAC allow-lists; tick front-end **`credentials: 'include'`** doc (normative **§3.3**). |
