...
**Last reviewed:** 2026-08-26 (repository snapshot below).
...
- [x] ✅ **POC — créneaux:** `website_config` key **`opening_hours`**, schema/seed, public GET, Admin GET/PATCH, **`useOpeningHours`**; **in-place edit on Infos** (Admin gear beside the section title); Contact **Quand ?** read-only from the same list.
- [x] ✅ **Tests** for opening-hours schema, get/patch use cases, and patch body parser.
- [ ] Remaining **`website_config` keys** + schemas for homepage welcome and Infos intro / tarifs / philosophy.
- [ ] **Admin APIs** + UI for those remaining content blocks.
- [ ] Public pages (`app/pages/index.vue`, Infos prologue, `IntroductionSection.vue`, tarifs, philosophy) load from public config with sensible seed defaults.
- [ ] **Tests** for remaining content-block schemas and use cases.
