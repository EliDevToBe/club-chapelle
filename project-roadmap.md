...
**Last reviewed:** 2026-09-05 (repository snapshot below; member roster admin on `invite-team`).
...
- [x] ✅ **Member roster edit / shell delete** — inline `public_name` edit (`PATCH /api/archers/:id`) + unlinked shell delete with participation cascade (`DELETE /api/archers/:id`, Admin Gestion du club). Competition browse combobox remains create-only for archers.
...
| Date       | Change                                                                 |
| ---------- | ---------------------------------------------------------------------- |
| 2026-09-05 | **RBAC:** removed implicit **`developer`** bypass from `requireRoles`; maintainer seed user gets **`developer`** + **`admin`**; use **`requireDeveloper`** for dev-only routes. |
