...
**Last reviewed:** 2026-07-04 (repository snapshot below).
...
- [ ] **Server-side Facebook fetch** (Graph API or approved alternative) with **caching** and env-based credentials (page ID, access token—never exposed to the client).
...
- [ ] **`website_config` key** + DTO for site settings (contact email, club address, Instagram/Facebook profile URLs).
- [ ] **Admin GET/PATCH** under `/api/admin/website-config/…` (**Admin** role per §3.2).
- [ ] **Public GET** for display values (no secrets).
- [ ] Refactor **Contact** and **`SocialSection.vue`** to read public settings with env/runtime fallback.
- [ ] **Admin UI** under existing **Gestion du site** (`WebsitePanel`).
- [ ] Reduce reliance on hard-coded mail/address in `app/pages/contact/index.vue`.
