...
- **Password change invalidates other sessions:** `auth_user.password_changed_at` is set whenever the password is changed (`POST /api/me/change-password` or reset-password). On each request, the auth middleware rejects access and refresh JWTs whose `iat` is **before** `password_changed_at`. After a successful change-password, the current browser receives **new** session cookies so it stays signed in.
