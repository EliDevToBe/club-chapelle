...
| View and update **own** profile (`auth_user.name`, e-mail via OTP, password)                           | ✅     | ✅      | ✅    |
| **Revoke own** access (password step-up; no Archer offboard)                                           | ✅     | ✅      | ✅    |
...
- **Admin** may **revoke** and **unlink** **other** accounts from the **Archer** shell (preserving history). A signed-in user may **revoke their own** access from `/settings` (current password required; same unlink / disable-login persistence; **no** Archer offboard). The **last Admin** cannot self-revoke.
...
- **Session snapshot for the client:** `GET /api/auth/session` returns `{ session: null }` when unauthenticated, or `{ session: { id, name, public_name, roles } }` when authenticated (aligned with `shared/auth/session-user.ts`; `public_name` is the first linked non-offboarded Archer, else `null`). E-mail is **not** in the session; the signed-in user loads it from `GET /api/me`. `POST /api/auth/login` responds with **`{ ok: true, session }`** after setting cookies, so the client can update UI without an extra round trip.
...
- **Password recovery:** `POST /api/auth/forgot-password` sends a transactional e-mail with a **recovery JWT** (separate verification from session access tokens) and records a **`token`** row (`forgot_password`). `POST /api/auth/reset-password` updates `auth_user.password` and `auth_user.password_changed_at`, sets that row’s **`used_at`** in one transaction, then issues new access and refresh cookies like a successful login.
- **Password change invalidates other sessions:** `auth_user.password_changed_at` is set whenever the password is changed (`POST /api/me/change-password` or reset-password). On each request, the auth middleware rejects access and refresh JWTs whose `iat` is **before** `password_changed_at`. After a successful change-password, the current browser receives **new** session cookies so it stays signed in. **Settings change-password only:** `POST /api/me/change-password` also sends a security e-mail via a Mailtrap template (`passwordChanged`; placeholder UUID in code until the live template exists) warning that the password changed and including a one-click **`recovery_link`** (`/reset-password?t=…`, same `forgot_password` JWT + `token` row as the forgot-password flow). Template variables: `user_name`, `user_email`, `recovery_link`, `privacy_policy_url`. Not sent after forgot-password reset (the user already initiated that flow). Mailtrap must be configured (same as invitations). Delivery failures are logged in the use case and never block the password change.
- **Member invitation:** `POST /api/invitations` (**Admin**) creates an invited `auth_user` (`authenticated: false`) and a linked Archer, issues an **invitation JWT** (`token_type.invitation`, 7 days; not valid as a session access token), and sends template mail. `POST /api/invitations/bind-archer` (**Admin**) creates a **fresh** invited `auth_user` (member role) linked to an **existing unlinked** Archer shell and sends the same mail/token flow; clears `offboarded_at` when re-inviting an archived shell. An `auth_user` row **always** has a linked Archer while it exists. `POST /api/auth/accept-invitation` sets the password, marks the user authenticated, consumes the token, and issues session cookies. See §3.2 (Admin-only delivery).
- **Member revoke:** `POST /api/users/:id/revoke` (**Admin**) unlinks all Archers from the user, then **deletes** the `auth_user` row (roles and tokens cascade); Archer shells and participations remain. Admin self-revoke is rejected. **Own revoke:** `POST /api/me/revoke` (authenticated; current password) uses the same persistence; the last Admin is rejected. Session JWTs are not blocklisted (see revocation limits below).
- **Own profile:** `GET` / `PATCH /api/me` (name), `POST /api/me/change-password`, and e-mail change via current password + one-time code mailed to the **new** address (`change_email` token, 15 minutes; confirm / resend / cancel on `/settings`). The last Admin cannot self-revoke.
- **Revocation limits:** Password change (or reset) invalidates **all** existing session JWTs via `password_changed_at` (see above). Revoke deletes the `auth_user` row, which blocks further API access. There is still no per-device session list or token blocklist beyond that.
...
- When a **Member** account is **revoked**, the **`auth_user` is deleted** and the Archer becomes an unlinked shell; the **Archer** and past participations remain for audit and continuity.
- **Admin** may **delete** an **unlinked shell** Archer only after **archiving** it (`POST /api/archers/:id/offboard` sets `offboarded_at`; participations are kept). **Hard delete** (`DELETE /api/archers/:id`) removes an **archived** unlinked shell and its participations in one transaction. Deletion is rejected when the Archer is not archived or still linked to an account (revoke first). **Re-invite** from the archived roster (`POST /api/invitations/bind-archer`) creates a fresh `auth_user`, links it to the shell, and clears `offboarded_at`.

### 4.2 Member (authenticated user)

A **Member** is a user account that is **linked** to an Archer while it exists. The shipped **Admin** invite flow **creates** a new Archer (`public_name` = invitee name) and links it to the new account. **Admin** may also invite an **existing unlinked Archer shell** (`POST /api/invitations/bind-archer`) from the member roster by creating a fresh account and sending the invitation mail.
