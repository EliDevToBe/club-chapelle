...
- **Member revoke:** `POST /api/users/:id/revoke` (**Admin**) unlinks all Archers from the user, clears the password, sets `authenticated: false`, and revokes unused tokens; the `auth_user` row and Archer history remain. Admin self-revoke is rejected. **Own revoke:** `POST /api/me/revoke` (authenticated; current password) uses the same persistence; the last Admin is rejected. Session JWTs are not blocklisted (see revocation limits below).
...
- **Revocation limits:** Password change (or reset) invalidates **all** existing session JWTs via `password_changed_at` (see above). Revoke (clears password + `authenticated: false`) also blocks further API access. There is still no per-device session list or token blocklist beyond that.
