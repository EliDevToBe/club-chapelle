...
- **Password change invalidates other sessions:** `auth_user.password_changed_at` is set whenever the password is changed (`POST /api/me/change-password` or reset-password). On each request, the auth middleware rejects access and refresh JWTs whose `iat` is **before** `password_changed_at`. After a successful change-password, the current browser receives **new** session cookies so it stays signed in. **Settings change-password only:** `POST /api/me/change-password` also sends a **best-effort security e-mail** via a Mailtrap template (`passwordChanged`; placeholder UUID in code until the live template exists) warning that the password changed and including a one-click **`recovery_link`** (`/reset-password?t=…`, same `forgot_password` JWT + `token` row as the forgot-password flow). Template variables: `user_name`, `user_email`, `recovery_link`, `privacy_policy_url`. Not sent after forgot-password reset (the user already initiated that flow). Missing mail configuration or delivery failures are logged and never block the change.
